...
const capitalize = (str) => {
  ...
  return (firstLetter + letter.slice(1).join(''))
};
...
const calculateDogAge = (pupsAge) => {
  const humanAge =  pupsAge * 7;
...
const isPangram = (sentence) => {
  let pangram = sentence.toLowerCase();
  let alphabet = 'abcdefghijklmnopqrstuvwxyz'.split('');
  for(let i = 0; i < alphabet.length; i++){
    if(pangram.indexOf(alphabet[i]) < 0){
      return false
    }
  };
  return true;
